...
export default function ChannelList({ currentServer, setCurrentChannel, addChannel }: ChannelListProps) {
  ...
  return (
    <div className="w-64 bg-white p-4 flex flex-col rounded-2xl shadow-lg">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold text-gray-800">{currentServer.name}</h2>
      </div>
      <div className="flex-grow">
        {currentServer.channels.map((channel) => (
          <button
            key={channel.id}
            className="w-full text-left p-2 rounded-xl flex items-center gap-2 bg-green-100 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all mb-2 text-gray-800"
            ...
            <form onSubmit={handleAddRoom}>
              ...
              <div className="mb-4">
                ...
                <select
                  id="roomType"
                  value={newRoomType}
                  onChange={(e) => setNewRoomType(e.target.value as "text" | "voice")}
                  className="w-full p-2 rounded-xl bg-gray-100 text-gray-800 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] focus:translate-x-[2px] focus:translate-y-[2px] focus:shadow-none transition-all"
                >
